...
class FlipSettings {
  ...
  ///
  /// Upstream painted every page ground, and the sheet mid-turn, in hardcoded
  /// white — right for a white book, wrong for a Mushaf read at night, where a
  /// white flash is the brightest thing on the screen. Mutable, like
  /// [startPageIndex], so a reader switching to night mode repaints on the new
  /// ground without rebuilding the book.
  Color paperColor;

  /// Whether the paper follows the finger while it is down.
  ///
  /// With this off the page does not move at all until the swipe is finished,
  /// and then turns in one animation. A Mushaf reader wants that: a resting
  /// thumb should never lift the corner of the page being read.
  final bool interactiveFold;
  ...
  FlipSettings copyWith({
    ...
    return FlipSettings(
      ...
      interactiveFold: interactiveFold ?? this.interactiveFold,
